...
/**
 * 基于Lucene8.4.0的文件搜索demo
 */
public class FileSearchDemo {
//    public static String INDEX_PATH = "D:\\study\\content\\lucene"; // 存放Lucene索引文件的位置
//    public static String SCAN_PATH = "D:\\study\\content\\text2"; // 需要被扫描的位置，测试的时候记得多在这下面放一些文件

    /**
     * 创建索引
     */
    public void creatIndex(String indexPath,String scanPath) {
        ...
        try {
            ...
            // 获取被扫描目录下的所有文件，包括子目录
            List<File> files = FileUtil.listAllFiles(scanPath);
     ...
     * 合并索引文件
     */
    public void mergeIndexFiles(String fromPath, String toPath) {
        IndexWriter writer = null;
        Directory directory = null;
        Directory tempDir = null;
        try {
            Path path = new File(toPath).toPath();
            directory = FSDirectory.open(path);
            Path temp = new File(fromPath).toPath();
